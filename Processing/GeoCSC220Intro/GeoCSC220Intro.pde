...
/* Purpose: Display 3D shapes, and augment them using multiple unique methods. 
/*          These methods will rotate, translate, scale, and shear x,y,z axis 
/*          properties of my generated 3D shapes. The end result is a randomly
/*          generated set of shapes that morph and move around the sketch.
 *********************************************************/
...
void setup() {
  ...
  // UNCOMMENT MAKE THE NEXT LIN E FIRST FOR FULL SCREEN
  //fullScreen(P3D);
  ...
  // Added Z axis parameter for 3D shapes. Randomly generating the Z axis between -100 and 100.
  for (int i = 0 ; i < dealy.length ; i++ ) {
    dealy[i] = new GeometricDealy((int)random(0,width), (int)random(0, height), (float)random(-100, 100),
      (int)random(50, width*.08), (int)random(50, height*.08),
      (int)random(0,256), (int)random(0,256), (int)random(0,256), (float)random(0, 100));
  }
}
...
void draw() {
  ...
  // Added perspective() mode and ortho() mode. Press 'c' to switch between the two.
  if (isPerspective) {
    perspective();
  }
  else
  ...
  if (isdemo) {
    if (demodealy == null) {
      demodealy =  new GeometricDealy(width/2,height/2, (float)random(-100, 100),
        width/4, height/4, 128, 128, 128, (float)random(0, 100));
    }
  ...
  // I liked the low alpha shape trails so i kept it that way.
  demodealy = null ;  
  fill(0,0,0,15);
  rectMode(CENTER);
...
void keyPressed() {
  if (key == 'c') {
   ...
   println("Perspective? " + isPerspective);
  }
  if (key == 'f') {
    isfrozen = ! isfrozen ;
  ...
  if (key == 'r' && !(isdemo || isfrozen)) {
    for (int i = 0 ; i < dealy.length ; i++) {
      dealy[i] = new GeometricDealy((int)random(0,width), (int)random(0, height), (float)random(-100, 100),
        (int)random(50, width*.08), (int)random(50, height*.1),
        (int)random(0,256), (int)random(0,256), (int)random(0,256), (float)random(0, 100));
    }
  }
}